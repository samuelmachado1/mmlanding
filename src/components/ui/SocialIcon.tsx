...
function iconClassName(platform: SocialPlatform, className: string, monochrome?: boolean) {
  const colorClass = monochrome ? 'text-brand-black' : platformColorClasses[platform];
  return [colorClass, className].filter(Boolean).join(' ');
}
...
export function SocialIcon({ platform, className = 'h-5 w-5', monochrome }: SocialIconProps) {
  switch (platform) {
    case 'instagram':
      return (
        <svg {...svgProps} className={iconClassName(platform, className, monochrome)}>
    ...
    case 'x':
      return (
        <svg {...svgProps} className={iconClassName(platform, className, monochrome)}>
          <path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83-7.254-8.622h7.406l5.236 6.923 6.086-7.923zm-1.291 19.54h2.036L6.486 3.24H4.298l13.312 17.453z" />
        </svg>
      );
    default: {
