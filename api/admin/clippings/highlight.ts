...
import { setHighlightItem } from '../../lib/store.ts';
...
export default async function handler(
  ...
  try {
    ...
    console.error('POST /api/admin/clippings/highlight failed:', error);
    return res.status(500).json({ error: 'Failed to set highlight' });
  }
}
