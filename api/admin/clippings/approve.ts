...
import { approveItem } from '../../lib/store.ts';
...
export default async function handler(
  ...
  try {
    ...
    console.error('POST /api/admin/clippings/approve failed:', error);
    return res.status(500).json({ error: 'Failed to approve clipping' });
  }
}
